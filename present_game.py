...
def update_paddle_heights():
    ...
    LPAD_HEIGHT = max(
        MIN_PAD_HEIGHT,
        min(MAX_PAD_HEIGHT, PAD_HEIGHT + l_change)
    )
    RPAD_HEIGHT = max(
        MIN_PAD_HEIGHT,
        min(MAX_PAD_HEIGHT, PAD_HEIGHT + r_change)
    )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if PLAY_AI.checkForInput(MENU_MOUSE_POS):
                    init()
                    AI_player = True if PLAY_AI.checkForInput(MENU_MOUSE_POS) else False   # AI LOGIC
                    play()
